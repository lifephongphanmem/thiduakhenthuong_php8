...
class tnhosodenghikhenthuongcongtrangController extends Controller
{
    ...
    public function ThongTin(Request $request)
    {
        ...
        $inputs['url_thamdinh'] = '/KhenThuongCongTrang/TiepNhan/DsHoSoThamDinh';
    ...
    public function TraLai(Request $request)
    {
        ...
        $url_tl = '/KhenThuongCongTrang/HoSo/ThongTin';
        ...
        storeThongBao($url_tl, $noidung, $chucnang, $inputs['mahoso'], null, $model->madonvi, $model->madonvi_xd, 'quanly', $tk_dn, 'dshosodenghikhenthuongcongtrang');
    ...
    public function ChuyenChuyenVien(Request $request)
    {
        ...
        $chucnang = 'congtrang';
        storeThongBao($url, $noidung, $chucnang, $inputs['mahoso'], null, $model->madonvi, session('admin')->madonvi, 'quanly', $inputs['tendangnhap_tn'], 'tnhosodenghikhenthuongcongtrang');
    ...
    public function ThamDinhHoSo(Request $request)
    {
        $inputs=$request->all();
        if(isset($inputs['hoso_tapthe'])){
            $a_hoso_tapthe = array_keys($inputs['hoso_tapthe'] ?? []);
            dshosothiduakhenthuong_tapthe::wherenotin('id', $a_hoso_tapthe)->update(['ketqua' => 0]);
        }
        if(isset($inputs['hoso_canhan'])){
            $a_hoso_canhan = array_keys($inputs['hoso_canhan'] ?? []);
            dshosothiduakhenthuong_canhan::wherenotin('id', $a_hoso_canhan)->update(['ketqua' => 0]);
        }
        if(isset($inputs['hoso_hogiadinh'])){
            $a_hoso_hogiadinh = array_keys($inputs['hoso_hogiadinh'] ?? []);
            dshosothiduakhenthuong_hogiadinh::wherenotin('id', $a_hoso_hogiadinh)->update(['ketqua' => 0]);
        }

        return redirect('/KhenThuongCongTrang/TiepNhan/ThongTin');
    }
}
