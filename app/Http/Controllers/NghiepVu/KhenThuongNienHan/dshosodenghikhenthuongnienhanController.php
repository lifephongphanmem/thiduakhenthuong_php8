...
class dshosodenghikhenthuongnienhanController extends Controller
{
    public static $url = '/KhenThuongNienHan/HoSo/';
    ...
    {
        if (!chkPhanQuyen('dshosodenghikhenthuongnienhan', 'danhsach')) {
            return view('errors.noperm')->with('machucnang', 'dshosodenghikhenthuongnienhan')->with('tenphanquyen', 'danhsach');
        }
        $inputs = $request->all();
        ...
        $inputs['trangthai'] = session('chucnang')['dshosodenghikhenthuongnienhan']['trangthai'] ?? 'CC';
        ...
        if ($inputs['trangthai'] == 'CC') {
            ...
            $inputs['url_hs'] = '/KhenThuongNienHan/HoSo/';
            $inputs['url_xd'] = '/KhenThuongNienHan/HoSo/';
            $inputs['url_qd'] = '/KhenThuongNienHan/HoSo/';
        }
    ...
    {
        if (!chkPhanQuyen('dshosodenghikhenthuongnienhan', 'thaydoi')) {
        ...
        return view('NghiepVu.KhenThuongNienHan.HoSoKhenThuong.ThayDoi')
            ->with('model', $model)
    ...
    public function XemHoSo(Request $request)
    {
        ...
        return view('NghiepVu.KhenThuongNienHan.HoSoKhenThuong.Xem')
            ->with('model', $model)
            ->with('model_canhan', $model_canhan)
            ->with('model_tapthe', $model_tapthe)
            ->with('model_detai', $model_detai)
    ...
    {
        if (!chkPhanQuyen('dshosodenghikhenthuongnienhan', 'hoanthanh')) {
            return view('errors.noperm')->with('machucnang', 'dshosodenghikhenthuongnienhan')->with('tenphanquyen', 'hoanthanh');
        }
        $inputs = $request->all();
        $model = dshosothiduakhenthuong::where('mahosotdkt', $inputs['mahoso'])->first();
        $inputs['trangthai'] = getTrangThaiChuyenHS(session('chucnang')['dshosodenghikhenthuongnienhan']['trangthai'] ?? 'CC');
        $inputs['thoigian'] = date('Y-m-d H:i:s');
        $inputs['lydo'] = ''; //Xóa lý do trả lại
        setChuyenDV($model, $inputs);

        //Thêm dữ liệu vào bảng thông báo
        $url = '/KhenThuongNienHan/TiepNhan/ThongTin';
        ...
        $chucnang = 'nienhan';
        storeThongBao($url, $noidung, $chucnang, $inputs['mahoso'], null, $model->madonvi, $inputs['madonvi_nhan'], 'quanly', null, 'tnhosodenghikhenthuongnienhan');
    ...
    function htmlCaNhan(&$result, $model)
    {
        if (isset($model)) {
            ...
            foreach ($model as $tt) {
                ...
                $result['message'] .= '<td class="text-center">' . $tt->chucvu . ',' . $tt->tenphongban . ',' . (array_key_exists($tt->tencoquan, getDsCoQuan()) ? getDsCoQuan()[$tt->tencoquan] : $tt->tencoquan) . '</td>';
                $result['message'] .= '<td class="text-center"> ' . ($tt->madanhhieukhenthuong) . '</td>';
                // $result['message'] .= '<td class="text-center"> ' . ($a_danhhieutd[$tt->madanhhieutd] ?? '') . '</td>';
