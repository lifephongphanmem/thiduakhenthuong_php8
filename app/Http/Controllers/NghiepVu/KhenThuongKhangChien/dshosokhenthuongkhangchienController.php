...
namespace App\Http\Controllers\NghiepVu\KhenThuongKhangChien;
...
class dshosokhenthuongkhangchienController extends Controller
{
    public static $url = '/KhenThuongKhangChien/HoSoKT/';
    ...
    {
        if (!chkPhanQuyen('dshosokhenthuongkhangchien', 'danhsach')) {
            return view('errors.noperm')->with('machucnang', 'dshosokhenthuongkhangchien')->with('tenphanquyen', 'danhsach');
        }
        $inputs = $request->all();
        ...
        $m_donvi = getDonVi(session('admin')->capdo, 'dshosokhenthuongkhangchien');
        $a_diaban = array_column($m_donvi->toArray(), 'tendiaban', 'madiaban');

        $inputs['madonvi'] = $inputs['madonvi'] ?? $m_donvi->first()->madonvi;
        $donvi = $m_donvi->where('madonvi', $inputs['madonvi'])->first();
        $inputs['maloaihinhkt'] = session('chucnang')['dshosokhenthuongkhangchien']['maloaihinhkt'] ?? 'ALL';
        $inputs['trangthai'] = session('chucnang')['dshosokhenthuongkhangchien']['trangthai'] ?? 'CC';
        ...
        $model = dshosothiduakhenthuong::where('madonvi', $inputs['madonvi'])
            ...
            ->wherein('phanloai', ['KTCHONGMY','KTCHONGPHAP'])
        ...
        //dd($inputs);
        return view('NghiepVu.KhenThuongKhangChien.HoSoKT.ThongTin')
            ->with('model', $model)
            ...
            ->with('a_phanloaihs', getPhanLoaiHoSo('KHANGCHIEN'))
            ->with('a_loaihinhkt', array_column($m_loaihinh->toArray(), 'tenloaihinhkt', 'maloaihinhkt'))
            ->with('inputs', $inputs)
            ->with('pageTitle', 'Danh sách hồ sơ khen thưởng kháng chiến');
    }

    public function ThayDoi(Request $request)
    {
        if (!chkPhanQuyen('dshosokhenthuongkhangchien', 'thaydoi')) {
            return view('errors.noperm')->with('machucnang', 'dshosokhenthuongkhangchien')->with('tenphanquyen', 'thaydoi');
        }
        $inputs = $request->all();
        $inputs['url'] = '/KhenThuongKhangChien/HoSoKT/';
        $inputs['url_hs'] = '/KhenThuongKhangChien/HoSoKT/';
        $inputs['url_qd'] = '/KhenThuongKhangChien/HoSoKT/';
        $inputs['phanloaihoso'] = 'dshosothiduakhenthuong';
        $inputs['phanloaikhenthuong'] = 'KHENTHUONG';
        ...
        $a_dhkt_canhan = getDHTDVaHinhThucKT('KHANGCHIEN');
        // $a_dhkt_tapthe = getDanhHieuKhenThuong($donvi->capdo, 'TAPTHE');
        $a_dhkt_tapthe = getDHTDVaHinhThucKT('KHANGCHIEN');
    ...
    public function XemHoSo(Request $request)
    {
        ...
        return view('NghiepVu.KhenThuongKhangChien.HoSoKT.Xem')
            ->with('model', $model)
            ->with('model_canhan', $model_canhan)
            ->with('model_tapthe', $model_tapthe)
            ->with('model_detai', $model_detai)
    ...
    public function InHoSoKT(Request $request)
    {
        ...
        return view('NghiepVu.KhenThuongKhangChien.HoSoKT.InHoSoKT')
            ->with('model', $model)
            ->with('model_canhan', $model_canhan)
            ->with('model_tapthe', $model_tapthe)
            ->with('model_detai', $model_detai)
    ...
    {
        if (!chkPhanQuyen('dshosokhenthuongkhangchien', 'hoanthanh')) {
            return view('errors.noperm')->with('machucnang', 'dshosokhenthuongkhangchien')->with('tenphanquyen', 'hoanthanh');
        }
        $inputs = $request->all();
    ...
    {
        $inputs = $request->all();
        $inputs['url'] = '/KhenThuongKhangChien/KhenThuong/';
        $inputs['url_hs'] = '/KhenThuongKhangChien/HoSoKT/';
        $inputs['url_xd'] = '/KhenThuongKhangChien/XetDuyet/';
        $inputs['url_qd'] = '/KhenThuongKhangChien/KhenThuong/';
        $inputs['phanloaihoso'] = 'dshosothiduakhenthuong';
        $inputs['mahinhthuckt'] = session('chucnang')['dshosokhenthuongkhangchien']['mahinhthuckt'] ?? 'ALL';
        $model = dshosothiduakhenthuong::where('mahosotdkt', $inputs['mahosotdkt'])->first();
        ...
        $inputs['khangchien']=true;
        $donvikhenthuongkhac=array_column(dmdonvikhenthuongkhac::where('madonvi_nhap',session('admin')->madonvi)->get()->toarray(),'tendonvi');
        $a_donvikt_v1 = array_unique(array_merge([$model->donvikhenthuong], getDonViKhenThuong()));
        $a_donvikt = array_unique(array_merge($a_donvikt_v1,$donvikhenthuongkhac));
