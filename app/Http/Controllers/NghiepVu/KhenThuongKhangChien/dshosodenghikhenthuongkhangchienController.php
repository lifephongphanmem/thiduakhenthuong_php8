...
class dshosodenghikhenthuongkhangchienController extends Controller
{
    public static $url = '/KhenThuongKhangChien/HoSo/';
    ...
    public function ThongTin(Request $request)
    {
        ...
        $inputs['url_return'] = static::$url . 'ThongTin';

        $m_donvi = getDonVi(session('admin')->capdo, 'dshosodenghikhenthuongkhangchien');
        $a_diaban = array_column($m_donvi->toArray(), 'tendiaban', 'madiaban');

        $inputs['madonvi'] = $inputs['madonvi'] ?? $m_donvi->first()->madonvi;
        $donvi = $m_donvi->where('madonvi', $inputs['madonvi'])->first();
        $inputs['maloaihinhkt'] = session('chucnang')['dshosodenghikhenthuongkhangchien']['maloaihinhkt'] ?? 'ALL';
        $inputs['trangthai'] = session('chucnang')['dshosodenghikhenthuongkhangchien']['trangthai'] ?? 'CC';
        ...
        if ($inputs['trangthai'] == 'CC') {
            ...
            $inputs['url_hs'] = '/KhenThuongKhangChien/HoSo/';
            $inputs['url_xd'] = '/KhenThuongKhangChien/XetDuyet/';
            $inputs['url_qd'] = '/KhenThuongKhangChien/KhenThuong/';
        } else {
            //Chỉ sử lý ở màn hình "ThongTin"
            $inputs['url_hs'] = '/KhenThuongKhangChien/HoSo/';
            $inputs['url_xd'] = '/KhenThuongKhangChien/HoSo/';
            $inputs['url_qd'] = '/KhenThuongKhangChien/HoSo/';
        }
        ...
        // dd($model);
        return view('NghiepVu.KhenThuongKhangChien.HoSoKhenThuong.ThongTin')
            ->with('model', $model)
    ...
    {
        if (!chkPhanQuyen('dshosodenghikhenthuongkhangchien', 'thaydoi')) {
            return view('errors.noperm')->with('machucnang', 'dshosodenghikhenthuongkhangchien')->with('tenphanquyen', 'thaydoi');
        }
        $inputs = $request->all();
        $inputs['url'] = '/KhenThuongKhangChien/HoSo/';
        $inputs['url_hs'] = '/KhenThuongKhangChien/HoSo/';
        $inputs['url_xd'] = '/KhenThuongKhangChien/XetDuyet/';
        $inputs['url_qd'] = '/KhenThuongKhangChien/KhenThuong/';
        $inputs['phanloaikhenthuong'] = 'KHENTHUONG';
        ...
        return view('NghiepVu.KhenThuongKhangChien.HoSoKhenThuong.ThayDoi')
            ->with('model', $model)
    ...
    public function XemHoSo(Request $request)
    {
        ...
        return view('NghiepVu.KhenThuongKhangChien.HoSoKhenThuong.Xem')
            ->with('model', $model)
            ->with('model_canhan', $model_canhan)
            ->with('model_tapthe', $model_tapthe)
            ->with('model_detai', $model_detai)
    ...
    {
        if (!chkPhanQuyen('dshosodenghikhenthuongkhangchien', 'hoanthanh')) {
            return view('errors.noperm')->with('machucnang', 'dshosodenghikhenthuongkhangchien')->with('tenphanquyen', 'hoanthanh');
        }
        $inputs = $request->all();
        $model = dshosothiduakhenthuong::where('mahosotdkt', $inputs['mahoso'])->first();
        $inputs['trangthai'] = getTrangThaiChuyenHS(session('chucnang')['dshosodenghikhenthuongkhangchien']['trangthai'] ?? 'CC');
        ...
        $chucnang = 'khangchien';
        storeThongBao($url, $noidung, $chucnang, $inputs['mahoso'], null, $model->madonvi, $inputs['madonvi_nhan'], 'quanly', null, 'tnhosodenghikhenthuongkhangchien');
