...
@section('custom-script-footer')
    ...
    <script>
        ...
        function add() {
            $('#frm_modify').find("[name='mahinhthucthidua']").attr('readonly', true);
            $('#frm_modify').find("[name='mahinhthucthidua']").val(null);
            $('#frm_modify').find("[name='tenhinhthucthidua']").val('');
        }
        ...
        function edit(id) {
            ...
            $.ajax({
                ...
                success: function(data) {
                    var form = $('#frm_modify');
                    form.find("[name='mahinhthucthidua']").attr('readonly', false);
                    form.find("[name='mahinhthucthidua']").val(data.mahinhthucthidua);                  
                    form.find("[name='tenhinhthucthidua']").val(data.tenhinhthucthidua);                  
                }
            });
        }
    </script>
...
@section('content')
    ...
    <div class="card card-custom" style="min-height: 600px">
        <div class="card-header flex-wrap border-1 pt-6 pb-0">
            <div class="card-title">
                <h3 class="card-label text-uppercase">Danh sách hình thức thi đua </h3>
            </div>
                        ...
                        <tbody>
                            ...
                            @foreach ($model as $ct)
                                <tr>
                                    ...
                                    <td>{{ $ct->mahinhthucthidua }}</td>
                                    <td>{{ $ct->tenhinhthucthidua }}</td>
                                    <td style="text-align: center">
                                        @if (chkPhanQuyen('dmhinhthucthidua', 'thaydoi'))
                                            ...
                                            <button title="Xóa thông tin" type="button"
                                                onclick="confirmDelete('{{ $ct->id }}','{{ '/HinhThucThiDua/Xoa' }}')"
    ...
    {!! Form::open(['url' => '/HinhThucThiDua/Them', 'id' => 'frm_modify']) !!}
                    ...
                    <div class="form-horizontal">
                        ...
                        <div class="form-group row">
                            <div class="col-12">
                                ...
                                {!! Form::text('tenhinhthucthidua', null, ['class' => 'form-control', 'required' => 'required']) !!}
                            </div>
                        </div>                        
                    </div>
                </div>
