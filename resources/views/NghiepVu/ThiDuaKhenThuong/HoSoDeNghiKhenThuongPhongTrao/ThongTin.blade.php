                    ...
                    <table class="table table-striped table-bordered table-hover" id="sample_3">
                        <thead>
                            <tr class="text-center">
                                ...
                                <th colspan="{{ count($a_trangthai_hoso) == 0 ? 1 : count($a_trangthai_hoso) }}">
                                    Số lượng hồ</br>sơ đề nghị</th>
                        ...
                        @foreach ($model as $key => $tt)
                            <tr>
                                ...
                                <td style="text-align: center">
                                    <a href="{{ url($inputs['url_hs'] . 'DanhSach?madonvi=' . $inputs['madonvi'] . '&maphongtraotd=' . $tt->maphongtraotd) }}"
                                        class="btn btn-icon btn-clean btn-lg mb-1 position-relative"
                                        title="Danh sách hồ sơ đề nghị khen thưởng">
                                        <span class="svg-icon svg-icon-xl">
                                            <i class="icon-lg la flaticon-folder-3 text-dark icon-2x"></i>
                                        </span>
                                        <span
                                            class="label label-sm label-light-danger text-dark label-rounded font-weight-bolder position-absolute top-0 right-0">{{ $tt->sohosodenghi }}</span>
                                    </a>

                                    <a href="{{ url($inputs['url_hs'] . 'DSHoSoThamGia?madonvi=' . $inputs['madonvi'] . '&maphongtraotd=' . $tt->maphongtraotd) }}"
                                        class="btn btn-icon btn-clean btn-lg mb-1 position-relative"
                                        title="Danh sách hồ sơ tham gia thi đua">
                                        <span class="svg-icon svg-icon-xl">
                                            <i class="icon-lg la flaticon-folder-2 text-dark icon-2x"></i>
                                        </span>
                                        <span
                                            class="label label-sm label-light-danger text-dark label-rounded font-weight-bolder position-absolute top-0 right-0">{{ $tt->sohosothamgia }}</span>
                                    </a>                                   
                                </td>                                
                            </tr>
                        @endforeach
